...
foreach ( $graph_lines_data as $line ) {
	$unit = ( '' !== $units ) ? ' <span class="vc_label_units"><span class="number-inner"'.$line['txtcolor'].'>' . $line['value'] . $units . '</span></span>' : '';
    $output .= '<p class="vc_label"' . $line['txtcolor'] . '>' . $line['label'] .'</p>';
	$output .= '<div class="vc_general vc_single_bar' . ( ( isset( $line['color'] ) && 'custom' !== $line['color'] ) ?
			' vc_progress-bar-color-' . $line['color'] : '' )
		. '">';
	$output .= '<small class="spacing"></small>';
	if ( $max_value > 100.00 ) {
