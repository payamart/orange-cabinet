 ...
 * @extends  WC_Widget
 */
class kitgreen_Widget_Attributes_Filter extends WC_Widget {

	/**
	 * Constructor.
	 */
	public function __construct() {
		$this->widget_cssclass    = 'woocommerce kitgreen_attributes_filter widget_layered_nav ';
		$this->widget_description = esc_html__( 'Shows a custom attribute in a widget which lets you narrow down the list of products when viewing product categories.', 'kitgreen' );
		$this->widget_id          = 'kitgreen_attributes_filter';
		$this->widget_name        = esc_html__( 'kitgreen Attributes Filter', 'kitgreen' );
		parent::__construct();
	}
	...
	public function init_settings() {
		...
		$this->settings = array(
			'title'      => array(
				'type'  => 'text',
				'std'   => esc_html__( 'Filter by', 'kitgreen' ),
				'label' => esc_html__( 'Title', 'kitgreen' )
			),
			'attribute'  => array(
				'type'    => 'select',
				'std'     => '',
				'label'   => esc_html__( 'Attribute', 'kitgreen' ),
				'options' => $attribute_array
			),
			'query_type' => array(
				'type'    => 'select',
				'std'     => 'and',
				'label'   => esc_html__( 'Query type', 'kitgreen' ),
				'options' => array(
					'and' => esc_html__( 'AND', 'kitgreen' ),
					'or'  => esc_html__( 'OR', 'kitgreen' )
				)
			),
		);
	}
	...
	public function widget( $args, $instance ) {
        ...
        ?> <div class="<?php echo $taxonomy;  ?>"><?php 
	...
	protected function layered_nav_list( $terms, $taxonomy, $query_type, $show_color ) {
		...
		foreach ( $terms as $term ) {
			...
			echo apply_filters( 'kitgreen_color_filter_nav_count', '<span class="count-atr">(' . absint( $count ) . ')</span>', $count, $term );
